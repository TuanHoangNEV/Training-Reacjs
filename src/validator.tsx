  ...
  const validator = (
    ...
    rules.forEach(r => {
      ...
      if (!isEmpty(value) && value.length < r.minLength!) {
        addErrors(r.key, `${r.label} must min length is ${r.minLength!}`);
      }
      if (!isEmpty(value) && value.length > r.maxLength!) {
        addErrors(r.key, `${r.label} must max length is${r.maxLength!}`);
      }
      if (!isEmpty(value) && parseInt(value, 10) < r.minValue!) {
        addErrors(r.key, `${r.label} min value is ${r.minValue!}`);
      }
      if (!isEmpty(value) && parseInt(value, 10) > r.maxValue!) {
        addErrors(r.key, `${r.label} max value is ${r.maxValue!}`);
      }
